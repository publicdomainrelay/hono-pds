...
function encodeValue(val: unknown, out: number[]): void {
  ...
  if (typeof val === "number") {
    if (Number.isInteger(val) && Number.isSafeInteger(val)) {
      ...
      throw new Error("DRISL-CBOR: floats not allowed");
    }
    return;
  }
  ...
  throw new Error(`DRISL-CBOR: unsupported type ${typeof val}`);
}
...
class Decoder {
  ...
  #readByte(): number {
    if (this.#pos >= this.#buf.length) throw new Error("DRISL-CBOR: unexpected end of input");
  ...
  #readHeader(): { major: number; value: number } {
    ...
    if (ai < 24) {
      ...
      if (!Number.isSafeInteger(value)) {
        throw new Error("DRISL-CBOR: 8-byte integer out of safe range");
      }
    } else {
      throw new Error(`DRISL-CBOR: unsupported additional info ${ai}`);
    }
    return { major, value };
  }
      ...
      case MAJOR_MAP: {
        ...
        for (let i = 0; i < value; i++) {
          ...
          if (typeof key !== "string") {
            throw new Error("DRISL-CBOR: map keys must be strings");
          }
          obj[key] = this.decode();
        }
        return obj;
      ...
      case MAJOR_TAG: {
        if (value === TAG_CID) {
          ...
          if (next.major !== MAJOR_BYTES) {
            throw new Error("DRISL-CBOR: tag 42 must wrap a byte string");
          }
        ...
        throw new Error(`DRISL-CBOR: unsupported tag ${value}`);
      }
      case MAJOR_SIMPLE: {
        switch (value) {
          case SIMPLE_FALSE:
          ...
          case SIMPLE_NULL:
            return null;
          default:
            throw new Error(`DRISL-CBOR: unsupported simple value ${value}`);
        }
      }
      default:
        throw new Error(`DRISL-CBOR: unsupported major type ${major}`);
    }
  }
